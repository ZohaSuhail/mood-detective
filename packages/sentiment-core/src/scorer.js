import { happy, sad, angry, negations, intensifiers, emojis, } from './lexicons.js';
...
export function analyze(sentence) {
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        if (weight !== 0) {
            ...
            const allowBoost = boost > 1 &&
                (!isNegated ||
                    (isNegated && ['this', 'that', 'it'].includes(nextToken)));
            if (allowBoost) {
                finalWeight =
                    Math.sign(finalWeight) * Math.ceil(Math.abs(finalWeight) * boost);
            }
